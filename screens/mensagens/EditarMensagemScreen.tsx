...
type Route = RouteProp<DrawerParamList, 'EditarMensagem'>;

export default function EditarMensagemScreen() {
  const navigation = useNavigation();
  const route = useRoute<Route>();
  const item = route.params.item;
  const [hospedagem, setHospedagem] = useState(String(item.hospedagem ?? ''));
  const [nome, setNome] = useState(String(item.nome ?? ''));
  const [email, setEmail] = useState(String(item.email ?? ''));
  const [telefone, setTelefone] = useState(String(item.telefone ?? ''));
  const [assunto, setAssunto] = useState(String(item.assunto ?? ''));
  const [mensagem, setMensagem] = useState(String(item.mensagem ?? ''));
  const [lida, setLida] = useState(Boolean(item.lida));
  const [salvando, setSalvando] = useState(false);

  async function salvar() {
    try {
      setSalvando(true);
      await api.put(`/mensagens/mensagens/${item.id}/`, {
